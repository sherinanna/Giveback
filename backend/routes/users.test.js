...
describe("GET /users/:username", function () {
  test("works for admin", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
      .set("authorization", `Bearer ${adminToken}`);
    expect(resp.body.user.firstName).toEqual("U1F");
  });
...
describe("POST /users/:username/events/:id", function () {
  test("works for admin", async function () {
    const resp = await request(app)
      ...
      .set("authorization", `Bearer ${adminToken}`);
    expect(resp.body).toEqual({ applied: testEventIds[1] });
  });
  ...
  test("not found for no such event", async function () {
    const resp = await request(app)
      .post(`/users/u1/events/0`)
      .set("authorization", `Bearer ${adminToken}`);
    expect(resp.statusCode).toEqual(404);
  });

  test("bad request invalid event id", async function () {
    const resp = await request(app)
      .post(`/users/u1/events/0`)
      .set("authorization", `Bearer ${adminToken}`);
    expect(resp.statusCode).toEqual(404);
